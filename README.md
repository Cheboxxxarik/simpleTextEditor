# simpleTextEditor

**simpleTextEditor** - это кроссплатформенный редактор .txt-файлов, написанный на языке программированиия Python с помощью библиотеки PyQt6.

## Установка
### 1-й способ
1. Установите [Python](https://www.python.org).
2. Установите [Git](https://git-scm.com/downloads).
3. Склонируйте репозиторий с помощью команды ```git clone https://github.com/Cheboxxxarik/simpleTextEditor```.
4. Откройте папку, куда вы склонировали репозиторий, в терминале.
5. Создайте виртуальное окружение Python с помощью команды ```python -m venv .venv```.
6. Активируйте виртуальное окружение.  
   **На Windows**: ```.venv/Scripts/activate```  
   **На Linux**: ```source .venv/bin/activate```
7. Установите библиотеку PyQt6 с помощью команды ```pip install pyqt6``` или ```pip3 install pyqt6```.
8. Запустите программу.  
   На Windows: ```python main.py```  
   На Linux и Mac: ```python3 main.py```
### 2-й способ (Для Windows)
1. Перейдите на вкладку Releases
### 3-й способ (Для Linux)
1. Перейдите на вкладку Releases
